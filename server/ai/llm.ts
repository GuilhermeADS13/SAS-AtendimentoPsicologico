...
export function clinicalSystemPrompt(ctx: AiAccessContext, requestedPatientId?: number, toolsEnabled = true, patientName?: string): string {
  return [
    ...
    toolsEnabled
      ? "As ações de escrita na agenda (agendar, remarcar, cancelar e registrar pagamento) exigem DUAS chamadas. Na primeira, chame a ferramenta SEM codigoConfirmacao: ela não executa nada, devolve o resumo da ação e um código. Apresente esse resumo ao(à) profissional em linguagem natural e espere a resposta. Somente depois de um 'sim' claro, na mensagem seguinte, chame a mesma ferramenta com os MESMOS parâmetros e com codigoConfirmacao igual ao código recebido. Nunca invente, adivinhe ou reaproveite um código, e nunca use o código na mesma mensagem em que a ação foi proposta: o servidor recusa. Se a pessoa mudar algum detalhe, recomece pela chamada sem código. Para remarcar, cancelar ou registrar pagamento, primeiro descubra o número da consulta consultando os agendamentos. Interprete e informe horários no horário de Brasília (fuso oficial do Brasil, sem horário de verão)."
      : "",
    toolsEnabled
