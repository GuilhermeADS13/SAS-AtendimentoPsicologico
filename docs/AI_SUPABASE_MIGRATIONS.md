...
Este procedimento aplica a migração `drizzle/migrations/0016_ai_message_feedback.sql` no banco PostgreSQL do Supabase sem colocar credenciais no Git, no frontend ou nos logs de CI.
...
A migração atual é idempotente para o enum, a tabela e os índices. A forma explícita de aplicá-la é:

```bash
cd /caminho/SAS-AtendimentoPsicologico
DATABASE_URL='postgresql://...valor-fornecido-pelo-segredo...' \
...
select typname from pg_type where typname = 'ai_feedback_rating';
select indexname
from pg_indexes
...
O resultado esperado é a tabela `aiMessageFeedback`, o enum `ai_feedback_rating` e os índices:
...
No provedor que executa o backend, cadastre `DATABASE_URL`, `LLM_BASE_URL`, `LLM_API_KEY`, `LLM_MODEL`, `LLM_TEMPERATURE`, `LLM_MAX_TOKENS` e `JWT_SECRET` como variáveis privadas. Não configure `DATABASE_URL` no projeto frontend nem em variáveis públicas.
